...
`load_dataset` can resolve dataset names through the default index.

```{python}
atdata.set_default_index(index)

ds = atdata.load_dataset("@local/docs-v1", TextSample, split="train")
...
```{python}
emb_ds = atdata.load_dataset("@local/embeddings-v1", EmbeddingSample, split="train")
...
for schema in index.list_schemas():
    print(f"  {schema.name:20s}  fields={list(schema.field_types.keys())}")
```
...
| Load by name | `atdata.load_dataset("name")` |
