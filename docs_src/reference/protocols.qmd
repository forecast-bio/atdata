...
- **Index**: Unified index for all backends (replaces deprecated AbstractIndex)
...
::: {.callout-warning}
The `AbstractIndex` protocol is deprecated. Use `atdata.Index` directly as the
type annotation. `AbstractIndex` is retained for backward compatibility and will
...
def list_all_datasets(index: Index) -> None:
    """Works with any Index instance."""
...
- `Index` (from `atdata`) — unified index supporting local and atmosphere backends
...
```{python}
#| eval: false
from atdata import Index, IndexEntry, Dataset
...
```{python}
#| eval: false
from atdata import IndexEntry, Index

# Check if object implements protocol
entry = index.get_dataset("test")
assert isinstance(entry, IndexEntry)

# Type hints work with Index
def process(index: Index) -> None:
...
# Function works with any Index instance
def count_datasets(index: Index) -> int:
...
atm_index = Index(atmosphere=client)
