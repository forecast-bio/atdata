...
| `manifests` | array\<[`#shardManifestRef`](#ac-foundation-dataset-record-shardmanifestref)\> | no | Per-shard manifest metadata for query-based access. Each entry corresponds to a shard in the storage union (matched by array index). Optional — datasets without manifests are still fully usable via sequential iteration. |

#### `ac.foundation.dataset.record#shardManifestRef` <span class='lexicon-badge lexicon-badge-object'>Object</span> {#ac-foundation-dataset-record-shardmanifestref}
