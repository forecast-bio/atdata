...
Unified interface implemented by `Index` regardless of backend:

```{python}
#| eval: false
def process_dataset(index: Index, name: str):
...
| **Protocols** | Backend abstraction | `IndexEntry`, `AbstractDataStore`, `DataSource` |
