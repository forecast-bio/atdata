...
The `Dataset` returned by `write_samples` is immediately iterable---or you
can load from existing tar files with `Dataset[T](url)`:
...
| `write_samples()` | Write and shard samples in one call |
...
The [Local Workflow](local-workflow.qmd) tutorial shows how to set up Index-managed storage for team-wide dataset discovery and sharing.
:::
